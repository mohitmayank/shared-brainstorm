// packages/server/src/install/gemini-cli.ts
...
export async function installGeminiCli(env: { home?: string } = {}): Promise<void> {
  const home = env.home ?? homedir();
  process.stdout.write(
    `\nshared-brainstorm — gemini-cli installer\n=========================================\n` +
      `gemini-cli does not yet have a standardised MCP config location.\n` +
      `Add the following to your gemini-cli MCP configuration manually:\n\n` +
      `${CONFIG_SNIPPET}\n\n`,
  );

  const promptsDir = join(home, '.shared-brainstorm', 'prompts');
  await mkdir(promptsDir, { recursive: true });
  const destPath = join(promptsDir, 'gemini-cli.md');
